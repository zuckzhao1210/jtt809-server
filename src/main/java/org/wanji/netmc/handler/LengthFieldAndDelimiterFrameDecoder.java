...
public class LengthFieldAndDelimiterFrameDecoder extends DelimiterBasedFrameDecoder {
     ...
     * @param lengthField  字段长度？
     * @param delimiters  头尾标识符号
     */
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) {
        // 可读字节小于偏移量，表示无法解析完整消息，返回null
        if (in.readableBytes() < this.lengthFieldEndOffset) {
            return null;
        } else {
            int actualLengthFieldOffset = in.readerIndex() + this.lengthFieldOffset;    // 真实的字段偏移位置
            int frameLength = this.getUnadjustedFrameLength(in, actualLengthFieldOffset, this.lengthFieldLength);   // 获取消息长度
            if (frameLength < 0) {
